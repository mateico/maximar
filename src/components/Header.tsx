...
export function Header() {
  ...
  const [pillRect, setPillRect] = useState<PillRect>({
    left: 0,
    top: 0,
    width: 0,
    height: 0,
  ...
  return (
    <header className="sticky top-0 z-50 bg-brand-yellow-100/85 backdrop-blur-md">
      <Container className="flex items-center justify-between px-0 py-[6.65px] md:py-3">
        <Link
          href="/"
        ...
        >
          <Image
            src="/main-logo-modified.png"
            alt="Maxymar Kiteschool"
            width={56}
            height={56}
            className="h-[45px] w-[45px] shrink-0 md:h-14 md:w-14"
            priority
          />
        </Link>
      ...
      {open && (
        <div
          ref={mobileMenuRef}
          className="absolute inset-x-0 top-full border-t border-ink-900/10 bg-brand-yellow-100 shadow-lg md:hidden"
        >
