...
export const metadata: Metadata = {
	...
	description: "Clases individuales, grupales, cursos de iniciación y alquiler de equipos de kitesurf en Playa Malvin, Montevideo.",
};
				...
				<Container>
					<div className="grid gap-8 lg:grid-cols-2">
						{SERVICES.map((service) => (
							...
							>
								<div className={`inline-flex h-12 w-12 items-center justify-center rounded-xl ${service.accent}`}>
									<service.icon className="h-6 w-6" />
								</div>
								<h2 className="mt-5 font-heading text-2xl font-bold text-ink-950">{service.title}</h2>
								<p className="mt-3 text-ink-600">{service.description}</p>
								<ul className="mt-5 space-y-2.5">
									{service.details.map((detail) => (
										<li key={detail} className="flex items-start gap-2.5 text-sm text-ink-700">
								...
								<div className="mt-7 flex flex-wrap items-center gap-4">
									<Button href="/contacto" variant="secondary" className="!px-5 !py-2.5 text-sm">
			...
			<section className="bg-brand-yellow-50 py-16 sm:py-20">
				<Container>
					<div className="rounded-3xl bg-ink-950 px-8 py-14 text-center sm:px-16">
						<h2 className="font-heading text-3xl font-bold text-white sm:text-4xl">¿No sabés cuál elegir?</h2>
						<p className="mx-auto mt-4 max-w-xl text-ink-300">
							Contanos tu experiencia previa y tus objetivos, y te recomendamos el mejor servicio para empezar.
						</p>
